...
1. Последовательно перезагрузите узлы (включая деплой-ноду), кроме вычислительных узлов с High-IOPS дисками (узлы из группы `vkcloud_scst_iscsi`): 
   ...
   После перезагрузки каждого узла убедитесь, что все сервисы загрузились и в работе {var(sys2)} нет ошибок.
   ...
   1. Перезагрузите следующий узел через 15 минут после успешной перезагрузки предыдущего.
      ...
      Перед перезагрузкой LM-узла убедитесь, что systemd unit Logstash сконфигурирован верно, иначе — установите значение для параметра `TimeoutStopSec` равное `300`.

      ```console
      $ grep 'TimeoutStopSec' /usr/lib/systemd/system/logstash-lm001.service
      TimeoutStopSec=300s
      ```
...
1. После перезагрузки управляющих узлов, убедитесь, что PostgreSQL-кластер успешно запущен:
   ...
   1. Получите список кластеров PostgreSQL:
      ...
      Выберите кластер и найдите в нем файл `stolonctl`:
      ...
      Получите полное название кластера из директории:
      ...
      Подставьте полученное название кластера в команду `stolonctl`:
   ...
   1. Если в выводе команды `HEALTHY` выводится состоянии `False`, выполните следующие команды на всех управляющих узлах:
