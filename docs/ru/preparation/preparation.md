...
Время создания и размер резервной копии зависят от объёма данных БД.
...
Чтобы создать резервный копии БД, выполните команды на одном управляющем узле:
...
$ sudo bash /srv/backup/backup_pg_projects_service.sh
...
В действиях, приведённых ниже, используются номерные экземпляры Tarantool первого управляющего узла: `breeze_01`, `dusk_01`.
...
Создайте бекап на одном из Controller-серверов:

```console
$ sudo etcdctl --endpoints=127.0.0.1:22379 snapshot save /srv/backup/iam-$(date +%Y%m%d-%H%M).db
```

В результате будет создан файл `iam-<DATE>.db`. Здесь `<DATE>` — дата создания резервной копии.
...
В результате будет создан файл `magnum-<DATE>.db`. Здесь `<DATE>` — дата создания резервной копии.
...
В результате будет создан файл `scrooge-<DATE>.db`. Здесь `<DATE>` — дата создания резервной копии.

### {heading(Apache ZooKeeper)[id=backup_apache_zookeeper]}

Для резервного копирования сохраните `/var/lib/zookeeper` на всех управляющих узлах любым удобным способом, например:
...
1. В выводе получите строку вида `<UUID>:<SEQNO>` с двумя значениями, разделёнными двоеточием:
...
Восстановление БД `mariadb-zabbix` на сервере мониторинга проведите аналогично процедуре на управляющих узлах, приведённой в разделе {linkto(#recovery_control_nodes)[text=%text]}.
...
Ниже приведён пример восстановления БД `magnum_addons`.
...
1. При наличии старой резервной копии, перенесите директорию резервных копий из Tarantool в `tarantool_old`, чтобы сохранить её:
...
```console
export NEW_RELEASE_NAME="4.1.0"
export PREV_RELEASE_NAME="4.0.2"
export NEW_DISTRIB_DIR="/home/redos/box-4.1.0"
...
Сделайте резервную копию Inventory предыдущего релиза:
...
В {var(system)} 4.1.0 нет типа эмуляции `Icelake-Client`. Если он используется в версии 4.0.2, замените его:
