...
1. Если требуется, переведите его в режим обслуживания согласно разделам **Руководства администратора {var(system)}**:
   ...
   * **Администрирование {var(sys2)}** → **Управление инфраструктурой, на которой развёрнута {var(sys1)}** → **Управление вычислительными узлами (гипервизорами)**.
   * **Администрирование {var(sys2)}** → **Управление инфраструктурой, на которой развёрнута {var(sys1)}** → **Операции с управляющими узлами**.
...
Пакеты обновления описаны в файлах:

* `packages-auto-cve.yaml` — переменные для Ansible, содержащие последние версии пакетов устранения уязвимостей.
* `list-packages-pre-update.yaml` — список пакетов, которые обновляются до общего обновления.
* `list-packages-auto-cve.yaml` — список пакетов с cve-уязвимостями. Создается запуском скрипта `create_packages_list.py` с использованием информации из `var_pre_update.yaml`.
...
1. Проверьте список серверов, на которых произойдет обновление:
   ...
   $ ansible-playbook -i inventory.yaml \
     ...
     -e nexus_host='<DEPLOY_NODE_FQDN>:8081' \
     update_os.yaml \
     --list-hosts
   ```

   Здесь `<DEPLOY_NODE_FQDN>` — значение переменной `deploy_node_fqdn` из файла `minimal.yml`.
...
1. Проверьте наличие роутеров на управляющих (Control Plane) или сетевых узлах (Network Node). Если роутеры есть, выполните их миграцию:
   ...
   Миграция заключается в удалении роутера из одного агента и добавлении на другой агент вручную. Во время миграции обмен данными между сетями, подключёнными к роутеру, будет нарушен.
   ...
   1. Проверьте, что роутер размещён на агенте, с которого будет выполнятся миграция:
   ...
   1. Удалите роутер с агента, на котором он расположен, и добавьте его на агент, выбранный ранее для миграции.
      ...
      Нагрузка на сервер, на котором размещён выбранный агент, возрастёт. Убедитесь, что ресурсов сервера достаточно для размещения дополнительного роутера.
   ...
   1. Проверьте, что пространство имён (namespace) роутера есть на узле, на который он был перенесён:
...
1. Последовательно перезагрузите серверы, кроме гипервизоров с High-IOPS дисками (хосты из группы `vkcloud_scst_iscsi`), предварительно убедившись в запуске всех сервисов и отсутствии ошибок в работе {var(sys2)} после перезагрузки каждого сервера:
   ...
   Это позволит автоматически перезапустить все поды Kubernetes-кластера без лишних ручных действий.

   </info>

   1. Зайдите на обновлённый сервер и перезагрузите его:
      ...
      Здесь `<SERVER_NAME>` — имя обновлённого сервера из файла `inventory.yaml`, созданного на предыдущих шагах.
...
После перезагрузки управляющего узла, убедитесь, что PostgreSQL-кластер успешно запущен.

Проверьте состояние кластера на одном из Controller-хосте:
